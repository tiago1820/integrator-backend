...
describe("Test de rutas", () => {
    ...
    describe("GET /rickandmorty/character/total", () => {
        it("Responde con status: 200", async () => {
            await agent.get("/rickandmorty/character/total").expect(200);
        });

        it("Responde con un objeto que tiene la propiedad 'total' de tipo number", async () => {
            const response = (await agent.get("/rickandmorty/character/total")).body;
            expect(response).toHaveProperty("total");
            expect(typeof response.total).toBe("number");
        });

        it("Si hay un error responde con status: 500", async () => {
            await agent.get("/rickandmorty/character/totalerror").expect(500);
        });
    })
